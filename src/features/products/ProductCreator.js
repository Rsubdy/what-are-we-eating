...
function ProductCreator() {
  ...
  const handleSubmit = (e) => {
    ...
    if (name === "") {
      ...
      handleAlert(
        `${name} is not a valid name of the product! Please use only letters.`,
        "danger"
      );
      setName("");
    } else {
    ...
    <Container>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <ToggleButtonGroup type="checkbox" size="sm" vertical className="mb-2">
          <Button
            variant="btn btn-warning btn-sm"
            className={glutenfree ? "" : "opacity-50"}
            dietname="glutenfree"
            onClick={handleDietExclusion}
          >
            Glutenfree
          </Button>
          <Button
            variant="btn btn-dark btn-sm"
            className={dairyfree ? "" : "opacity-50"}
            dietname="dairyfree"
            onClick={handleDietExclusion}
          >
            Dairyfree
          ...
          <Button
            ...
            className={vegetarian ? "" : "opacity-50"}
            dietname="vegetarian"
            onClick={handleDietExclusion}
          >
            Vegetarian
